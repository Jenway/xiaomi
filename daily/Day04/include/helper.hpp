...
void test_moveable(M c1, NM c2, long& value)
{
    ...
    using V2type = typename iterator_traits<typename NM::iterator>::value_type;
    timeStart
        = clock();
    for (long i = 0; i < value; ++i) {
        snprintf(buf, 10, "%d", rand());
...
inline string get_a_target_string()
{
    long target = 0;
    char buf[10];
    cout << "target (0~" << RAND_MAX << "): ";
...
inline int compareLongs(const void* a, const void* b)
{
